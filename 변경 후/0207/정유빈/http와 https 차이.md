...
- 만약 인증서에 적힌 CA의 이름과 브라우저가 소유하고 있는 CA 이름이 같다면 CA의 공개키로 SSL 인증서 복호화합니다.
- 이후 브라우저는 인증서의 유효성을 검사하고 세션키를 발급합니다.
- 이제 브라우저는 세션키를 보관하며 SSL 내부에 들어있던 서버의 공개키를 가지고 세션키를 암호화해서 서버에게 보냅니다.
- 서버 측은 가지고 있는 개인키로 세션키를 복호화하여 세션키를 얻습니다.
