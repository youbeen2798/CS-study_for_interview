...
2. 클라이언트는 토큰을 별도의 공간에 저장한 후, 서버에 요청마다 헤더에 토큰을 함께 보낸다.
3. 허버는 해당 JWT의 유효성을 검사하고 인가한다.

- 클라이언트에서 JWT를 저장하는 공간은 대표적으로 localStorage와 Cookie가 있다.

<h1> 토큰 장점 </h1>

1. JWT는 클라이언트 측에서 저장하므로 서버 메모리 혹은 DB에 부담을 주지 않으며, 서버는 완전히 무상태성(Stateless)로 남게 된다.
2. 클라이언트와 서버가 독립되기 때문에 확장성을 보장한다.

<h1> 토큰 단점 </h1>

1. JWT가 브라우저에 그대로 노출되어 있으므로 위변조, 손상의 위험이 크다.
2. 한 때 발급된 토큰은 임의로 만료시킬 수 없다. 따라서 토큰이 공격자에게 탈취되었다면, 공격자는 토큰이 만료될 때까지 계속 공격할 수 있다.
