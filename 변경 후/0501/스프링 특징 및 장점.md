...
![image](https://user-images.githubusercontent.com/62228401/235912848-ccc34407-595b-4f9f-bc44-261ed01a3fcd.png)
...
- 자, 이제 위의 문제가 해결되었다.
- 회원 가입과 회원 조회와 같은 핵심 관리 사항과 시간을 측정하는 공통 관심 사항을 분리하였다.
...
- 웹 서버는 정적인 데이터를 처리하는 서버로 단순 이미지 HTML을 처리하는 서버이며, WAS(Web Applicaion Server)는 동적인 데이터를 처리하는 서버로 DB 연동 데이터 조작 등과 같은 처리를 WAS에서 한다.
- 스프링 Boot의 기본 내장 WAS는 Apached Tomcat이다.
