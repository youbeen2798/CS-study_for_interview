...
### 두가지 간단한 예시
#### 공개키로 암호화, 개인키로 복호화
- A랑 B는 각각의 공개키와 개인키를 가지고 있다.
- A가 B의 공개키로 삼성 SW 문제를 암호화 해서 B에게 보내줬다.
- B는 자신의 개인키로 복호화 해서 열람이 가능하지만, A는 암호화는 했지만 복호화는 하지 못한다.
- 이때, A가 밝히지 않는 이상 B는 누가 보낸지 모른다.

</br>

#### 개인키로 암호화, 공개키로 복호화
- 삼성 그룹이 삼성 SW 문제를 개인키로 암호화 하였다.
- 시험치룰 취준생들은 삼성 SW 문제를 풀기 위해 삼성 공개키로 복호화해야한다.
- 이때, 삼성 그룹이 암호화 했다는 것을 알 수 있고(누가 암호화 했는지)
- 내가 인증을 할 수 있다는 것을 증명할 수 있음 -> 암호화 했다는 것을 증명할 수 있음
- 그래서 이방식은 전사서명으로 사용함

</br>

 공개키(안전함)  / 대칭키(빠름) 방식을 혼합해서 사용한다. 
