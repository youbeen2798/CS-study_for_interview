...
- 실시간 운영체제에서는 교착 상태를 막기 위해 "예방" 방법을 사용하고 있음

</br>

- UNIX, Window 등 대부분의 운영체제가 이 방법으로 사용한다. (범용 운영체제)
1. Windows와 UNIX 애플리케이션은 뮤텍스, 세마포어, 락 등과 같은 공유 자원을 사용한다. 이러한 자원을 관리하면서 발생하는 교착 상태는 예측하기 어렵기 때문에 예방, 회피, 회복, 탐색 보다는 무시 기법인 타임아웃이 적절한 경우가 많다.

2. 복잡한 운영체제다. -> 적용하기 어려움  / 무시는 구현이 간단하고, 대부분의 교착 상태를 해결할 수 있다. </br>
프로세스 감시하고, 교착 상태가 발생하면 프로그램을 종료  시키는 것으로 간단히 구현함

3. 무한정 기다리는 상황이 발생할 확률이 매우 낮음 ( 대부분의 시스템에서는 교착 상태가 발생하는 경우 드뭄)
-> 예방은 상대적으로 오버헤드가 크기 때문에 무시 방법을 사용하여 시스템의 성능을 높이는 것이 보편적임
